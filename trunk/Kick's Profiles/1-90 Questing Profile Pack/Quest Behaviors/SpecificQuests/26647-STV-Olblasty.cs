...
namespace Honorbuddy.Quest_Behaviors.SpecificQuests.Olblasty
{
    ...
    {
        public Olblasty(Dictionary<string, string> args)
            : base(args)
        {
            try
            {
        ...
        public WoWUnit timmah
        {
            get
            {
                return ObjectManager.GetObjectsOfType<WoWUnit>().Where(u => u.Entry == 43560 && u.IsAlive).OrderBy(u => u.Distance).FirstOrDefault();
            }
        }
        ...
        {
            return _root ?? (_root = new Decorator(ret => !_isBehaviorDone, new PrioritySelector(DoneYet, KillSoldier)));
        }
    }
}
