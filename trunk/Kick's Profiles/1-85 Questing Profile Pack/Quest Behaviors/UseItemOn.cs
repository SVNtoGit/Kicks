                    ...
                    case ObjectType.Npc:
                        var baseTargets = ObjectManager.GetObjectsOfType<WoWUnit>()
                                                               ...
                                                               .Where(target => !_npcBlacklist.Contains(target.Guid) && !BehaviorBlacklist.Contains(target.Guid)
                                                                                ...
                                                                                && MobIds.Contains((int)target.Entry) && !BlacklistIfPlayerNearby(target) && (!IgnoreMobsInBlackspots || Targeting.IsTooNearBlackspot(ProfileManager.CurrentProfile.Blackspots, target.Location)));
