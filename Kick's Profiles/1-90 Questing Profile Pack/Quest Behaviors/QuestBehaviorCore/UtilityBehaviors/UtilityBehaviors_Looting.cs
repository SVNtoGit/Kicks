...
namespace Honorbuddy.QuestBehaviorCore.UtilityBehaviors
{
    public class LootingPS : PrioritySelector
    {
        ...
        private WoWUnit SelectedLootMob { get; set; }
            ...
            return new List<Composite>()
            {
                new Decorator(context => CharacterSettings.Instance.LootMobs,
                    new PrioritySelector(
                        ...
                        new Decorator(context => !QuestBehaviorBase.IsViable(SelectedLootMob),
                            new ActionFail(context => { SelectedLootMob = NearestLootableMob(); })),

                        // If we found a mob to loot, go for it...
                        new Decorator(context => QuestBehaviorBase.IsViable(SelectedLootMob),
                            new PrioritySelector(
                                // If too far away, move to mob...
                                 new Decorator(context =>   SelectedLootMob.Distance > SelectedLootMob.InteractRange,
                                     new Action(context => { Navigator.MoveTo(SelectedLootMob.Location); }))
